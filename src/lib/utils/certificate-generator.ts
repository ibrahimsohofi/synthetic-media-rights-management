...
import { simulateBlockchainRegistration } from "../ethereum-utils";
...
export async function generateCertificate(
  ...
  // For premium certificates, register on blockchain if requested
  if (certificateType === "premium" && registerOnBlockchain && !work.blockchain) {
    try {
      // In a real application, this would connect to a real blockchain
      // For demo purposes, we'll simulate the blockchain registration
      const result = await simulateBlockchainRegistration(
        certificateId,
        work.metadataHash || ""
      );
      ...
      if (result.success) {
        ...
        metadata.networkName = result.networkName;
      }
    } catch (error) {
      console.error("Blockchain registration error:", error);
      // Continue without blockchain registration if it fails
    }
  }
